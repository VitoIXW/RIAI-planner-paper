...
class RttPlanner():
    ...
    def _q_nearest(self, q_rand):
        ...
        return self._tree[np.argmin(distances)]

    
    def _q_new(self, q_rand, q_nearest, speed=5.0):
        direction = q_rand[:3] - q_nearest[:3]
        norm = np.linalg.norm(direction)
        if norm < 1e-9:
            return None

        new_space = q_nearest[:3] + self._step_size * direction / norm
    ...
    def plan(self, start, goal, speed, obstacles, bias_prob=.0, limit=False, tol_space=2.0, tol_time=5.0):
        ...
        for n_iterations in range(self._n_steps):
            ...
            q_new = self._q_new(q_rand, q_nearest._position, speed)
            if q_new is None:
                continue
